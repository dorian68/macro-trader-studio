...
serve(async (req) => {
  ...
  try {
    ...
    if (!user) {
      return new Response(
        JSON.stringify({ error: authError || "Verified email required" }),
        {
    ...
    // The RPC atomically handles paid plans, deleted accounts, and the
    // commercial bypass for internal super users.
