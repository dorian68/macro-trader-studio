      ...
      <Tabs defaultValue="quick-access" className="h-full flex flex-col overflow-hidden">
        ...
        <TabsContent value="market-intel" className="flex-1 min-h-0 mt-2 overflow-hidden animate-fade-in">
          ...
          <div className="flex items-center justify-between pt-0.5 shrink-0">
            <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" disabled={currentPage === 0} onClick={() => setPage(p => Math.max(0, p - 1))}>
              <ChevronLeft className="h-3.5 w-3.5 mr-1" /> Prev
            </Button>
            <span className="text-xs text-muted-foreground">{currentPage + 1} / {totalPages}</span>
            <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" disabled={currentPage >= totalPages - 1} onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))}>
