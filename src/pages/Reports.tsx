...
export default function Reports() {
  ...
  const downloadPDF = () => {
    ...
    // Create a new window with the HTML content styled for printing
    ...
    if (printWindow) {
      printWindow.document.write(`
        ...
        <html>
          <head>
            <title>${reportConfig.title}${selectedAsset ? ` - ${selectedAsset.symbol}` : ''}</title>
            <meta charset="UTF-8">
            <style>
              * { box-sizing: border-box; }
              ...
              h1 { font-size: 28px; border-bottom: 3px solid #2563eb; padding-bottom: 10px; }
              h2 { font-size: 22px; }
              h3 { font-size: 18px; }
              p { margin: 0.75em 0; }
          ...
          <body>
            <div class="no-print" style="background: #eff6ff; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #2563eb;">
              <p style="margin: 0; font-weight: 600; color: #1e40af;">
                📄 Ready to save as PDF
              </p>
              <p style="margin: 5px 0 0 0; font-size: 14px; color: #4b5563;">
                Use <strong>Ctrl+P (Cmd+P on Mac)</strong> or the print button, then select "Save as PDF"
              </p>
            </div>
            ...
            <p style="text-align: center; font-size: 12px; color: #9ca3af;">
              This report was generated by Alphalens Research Platform<br>
              © ${new Date().getFullYear()} Alphalens. All rights reserved.
            </p>
          </body>
        </html>
      `);
      ...
      toast({
        title: "Popup Blocked",
        description: "Please allow popups for this site to download the PDF.",
        variant: "destructive"
      });
    }
  ...
  const sendByEmail = async () => {
    ...
    if (!reportConfig.email) {
      toast({
        title: "Email Required",
        description: "Please provide an email address to send the report.",
        variant: "destructive"
      });
      return;
    }
    ...
    toast({
      title: "Sending Email...",
      description: "Your report is being sent. Please wait.",
    });
    
    try {
      const { error } = await supabase.functions.invoke('send-report-email', {
        body: {
          to: reportConfig.email,
